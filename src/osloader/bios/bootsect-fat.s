...
bootCode:
	...
	;; The MBR or bootloader code will pass the boot device number to us in
	;; the DL register
	mov byte [DISK], DL

	mov dword [LOADERSECTOR], 0

	;; If we are not booting from a floppy, the MBR code should have put a
	;; pointer to the MBR record for this partition in SI.  Calculate the
	;; partition starting sector and add it to the LOADERSECTOR value
	cmp byte [DISK], 80h
	jb .noOffset
	mov EAX, dword [SI + 8]
...
;; The installation process will record the starting cluster of the loader, and
;; number of sectors to read, here.
LOADERSTARTCLUSTER	dd 0
LOADERNUMSECTORS	dd 0

;; This puts the value AA55h in the last two bytes of the boot sector.  The
;; BIOS uses this to determine whether this sector was meant to be booted from
;; (and also helps prevent us from making the boot sector code larger than 512
;; bytes)
ENDSECTOR			dw 0AA55h
