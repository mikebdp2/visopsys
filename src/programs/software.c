...
static connectInfo conn = { 0 };
static int connectThreadPid = 0;
...
static objectKey working(void (*cancelCallBack)(objectKey),
	const char *format, ...)
{
	// Generic working message code for either text or graphics modes
	...
	if (graphics)
	{
		dialogWindow = windowNewCancelDialog(window, _("Info"), output,
			cancelCallBack);
	}
	else
	{
...
static void connectThread(void)
{
	...
	status = connect(conn.fd, conn.addrInfo->ai_addr,
		conn.addrInfo->ai_addrlen);
...
static void connectThreadCancel(objectKey dialogWindow
	__attribute__((unused)))
{
	if (connectThreadPid && multitaskerProcessIsAlive(connectThreadPid))
	{
		// Kill our thread
		multitaskerKillProcess(connectThreadPid);
	}
...
static int openConnection(const char *host, int port)
{
	...
	status = getaddrinfo(host, portString, &hints, &conn.addrInfo);
	if (status)
	{
		status = errno;
		perror("socket");
	...
	if (conn.addrInfo->ai_family == AF_INET)
	{
		struct sockaddr_in *addr =
			(struct sockaddr_in *) conn.addrInfo->ai_addr;
	...
	else
	{
		struct sockaddr_in6 *addr6 =
			(struct sockaddr_in6 *) conn.addrInfo->ai_addr;
	...
	conn.fd = socket(conn.addrInfo->ai_family, conn.addrInfo->ai_socktype,
		conn.addrInfo->ai_protocol);
	if (conn.fd < 0)
	{
		status = errno;
		perror("socket");
	...
	dialogWindow = working(&connectThreadCancel, _("Trying %s:%d..."),
		addrString, port);

	connectThreadPid = multitaskerSpawn(&connectThread, "connect thread",
		0 /* no args */, NULL /* no args */, 1 /* run */);
	if (connectThreadPid < 0)
	{
		status = errno;
		perror("multitaskerSpawn");
		windowCancelDialogDestroy(dialogWindow);
		error("%s", _("Couldn't connect"));
		goto out;
	}

	while (multitaskerProcessIsAlive(connectThreadPid))
		multitaskerYield();
