...
This command will display network traffic to/from a named network device, if
specified, otherwise all devices.
...
static objectKey filterButton = NULL;
...
static unsigned linkPacketLen(int linkProtocol, unsigned char *packet,
	unsigned packetLen)
{
...
static void printTcp(unsigned char *buffer, unsigned len)
{
	...
	if (srcPort)
		snprintf(srcPortString, sizeof(srcPortString), " (%s)", srcPort);

	destPortString[0] = '\0';
	destPort = ipPort2String(ntohs(header->destPort));
	if (destPort)
		snprintf(destPortString, sizeof(destPortString), " (%s)", destPort);
...
static void printIp4(unsigned char *buffer)
{
	...
	inet_ntop(AF_INET, &header->srcAddress, srcString, sizeof(srcString));
	inet_ntop(AF_INET, &header->destAddress, destString, sizeof(destString));
...
static void printArp(unsigned char *buffer)
{
	...
	inet_ntop(AF_INET, &arp->srcLogicalAddress, srcString, sizeof(srcString));
	inet_ntop(AF_INET, &arp->destLogicalAddress, destString,
		sizeof(destString));
...
static void printIp6(unsigned char *buffer)
{
	...
	inet_ntop(AF_INET6, &header->srcAddress, srcString, sizeof(srcString));
	inet_ntop(AF_INET6, &header->destAddress, destString, sizeof(destString));
...
{
	// Assume IPv4 for the time being
...
{
	// Determine the link-level packet length
	packetLen = linkPacketLen(hook->linkProtocol, packet, packetLen);
	if (!packetLen)
	{
...
static void filterDialog(void)
{
	// This will pop up a dialog that allows the user to set and remove packet
	// type filters
	...
	typedef struct {
 	...
 	filterCheckbox filters[] = {
		{ "Link layer", NULL, NULL },
		{ "Ethernet", NULL, &globalFilter.ethernet },
		{ "Loop", NULL, &globalFilter.loop },
		{ "Network layer", NULL, NULL },
		{ "IPv4", NULL, &globalFilter.ip4 },
		{ "ARP", NULL, &globalFilter.arp },
		{ "IPv6", NULL, &globalFilter.ip6 },
		{ _("Unsupported"), NULL, &globalFilter.netUnsupp },
		{ "Transport layer", NULL, NULL },
		{ "ICMP", NULL, &globalFilter.icmp },
		{ "TCP", NULL, &globalFilter.tcp },
		{ "UDP", NULL, &globalFilter.udp },
		{ _("Unsupported"), NULL, &globalFilter.transUnsupp }
	};

	// Create the dialog
	dialogWindow = windowNewDialog(window, _("Show selected packets"));
	if (!dialogWindow)
		return;
	...
	for (count = 0; count < (int)(sizeof(filters) / sizeof(filterCheckbox));
		count ++)
	{
		if (filters[count].filter)
		{
			// Make a checkbox for it

			if (params.gridX >= (COLUMNS - 1))
			{
				params.gridX = 0;
				params.gridY += 1;
			}
			...
			filters[count].checkBox = windowNewCheckbox(dialogWindow,
				filters[count].text, &params);

			windowComponentSetSelected(filters[count].checkBox,
				(*filters[count].filter? 0 : 1));

			params.gridX += 1;
		}
		else
		{
			// It's a layer grouping.  Make a label for it.

			params.gridY += 1;
			params.gridWidth = COLUMNS;

			if (params.gridX)
			{
				params.gridX = 0;
				windowNewDivider(dialogWindow, divider_horizontal, &params);
				params.gridY += 1;
			}

			windowNewTextLabel(dialogWindow, filters[count].text, &params);
			params.gridY += 1;
		}
	}
	...
	container = windowNewContainer(dialogWindow, "button container", &params);

	// Create the OK button
	params.gridY = 0;
	params.padBottom = params.padRight = 0;
	params.gridWidth = 1;
	...
	while (1)
	{
		...
		{
			for (count = 0; count < (int)(sizeof(filters) /
				sizeof(filterCheckbox)); count ++)
			{
				if (filters[count].checkBox &&
					windowComponentGetSelected(filters[count].checkBox,
					&selected) >= 0)
				{
					*filters[count].filter = (selected? 0 : 1);
				}
			}
...
static int constructWindow(void)
{
	...
	params.gridY += 1;
	params.padBottom = 5;
	filterButton = windowNewButton(window, _("Filters"), NULL, &params);
	windowRegisterEventHandler(filterButton, &eventHandler);
...
static unsigned getNetworkType(int linkProtocol, unsigned char *packet,
	unsigned packetLen)
{
	// Return the network layer type

	networkEthernetHeader *ethernet = (networkEthernetHeader *) packet;
	unsigned short etherType = 0;

	if (linkProtocol == NETWORK_LINKPROTOCOL_ETHERNET)
	{
		if (packetLen >= sizeof(networkEthernetHeader))
		{
			etherType = ntohs(ethernet->type);

			if (etherType == NETWORK_ETHERTYPE_IP4)
				return (NETWORK_NETPROTOCOL_IP4);

			if (etherType == NETWORK_ETHERTYPE_ARP)
				return (NETWORK_NETPROTOCOL_ARP);

			if (etherType == NETWORK_ETHERTYPE_IP6)
				return (NETWORK_NETPROTOCOL_IP6);
		}
	}
	else if (linkProtocol == NETWORK_LINKPROTOCOL_LOOP)
	{
		// Assume IPv4 for the time being
		return (NETWORK_NETPROTOCOL_IP4);
	}

	return (NETWORK_NETPROTOCOL_UNSUPPORTED);
}


static unsigned getTransportType(int linkProtocol, unsigned char *packet,
	unsigned packetLen)
{
	// Return transport layer type

	unsigned transProto = NETWORK_TRANSPROTOCOL_UNSUPPORTED;
	unsigned netProto = NETWORK_NETPROTOCOL_UNSUPPORTED;
	unsigned char *buffer = packet;
	unsigned bufferLen = packetLen;
	networkIp4Header *ip4 = NULL;
	networkIp6Header *ip6 = NULL;

	// Get the network protocol type
	netProto = getNetworkType(linkProtocol, packet, packetLen);

	if (linkProtocol == NETWORK_LINKPROTOCOL_ETHERNET)
	{
		// Move past the Ethernet header
		buffer += sizeof(networkEthernetHeader);
		bufferLen -= sizeof(networkEthernetHeader);
	}

	// Get the transport protocol type

	if (netProto == NETWORK_NETPROTOCOL_IP4)
	{
		if (bufferLen >= sizeof(networkIp4Header))
		{
			ip4 = (networkIp4Header *) buffer;
			transProto = ip4->protocol;
		}
	}
	else if (netProto == NETWORK_NETPROTOCOL_ARP)
	{
		return (NETWORK_NETPROTOCOL_NONE);
	}
	else if (netProto == NETWORK_NETPROTOCOL_IP6)
	{
		if (bufferLen >= sizeof(networkIp6Header))
		{
			ip6 = (networkIp6Header *) buffer;
			transProto = ip6->nextHeader;
		}
	}

	// Return the transport protocol, if supported
	...
	switch (transProto)
	{
		...
		default:
			return (NETWORK_TRANSPROTOCOL_UNSUPPORTED);
	}
}


static int filter(int linkProtocol, unsigned char *packet, unsigned packetLen)
{
	// Return 1 if we're filtering out this type of packet
	...
	if (globalFilter.ethernet)
	{
		if (linkProtocol == NETWORK_LINKPROTOCOL_ETHERNET)
			return 1;
	}
	if (globalFilter.loop)
	{
		if (linkProtocol == NETWORK_LINKPROTOCOL_LOOP)
			return 1;
	}
	...
	if (globalFilter.arp)
	{
		if (netProto == NETWORK_NETPROTOCOL_ARP)
			return 1;
	}
	if (globalFilter.ip6)
	{
		if (netProto == NETWORK_NETPROTOCOL_IP6)
			return 1;
	}
	if (globalFilter.netUnsupp)
	{
		if (netProto == NETWORK_NETPROTOCOL_UNSUPPORTED)
			return 1;
	}
	...
	// Get the transport protocol type
	transProto = getTransportType(linkProtocol, packet, packetLen);

	if (globalFilter.icmp)
	{
		if (transProto == NETWORK_TRANSPROTOCOL_ICMP)
			return 1;
	}
	if (globalFilter.tcp)
	{
		if (transProto == NETWORK_TRANSPROTOCOL_TCP)
			return 1;
	}
	if (globalFilter.udp)
	{
		if (transProto == NETWORK_TRANSPROTOCOL_UDP)
			return 1;
	}
	if (globalFilter.transUnsupp)
	{
		if (transProto == NETWORK_TRANSPROTOCOL_UNSUPPORTED)
			return 1;
	}
...
static void sniffStream(deviceHook *hook, int input)
{
	...
	if (packetLen)
	{
		...
		if (filter(hook->linkProtocol, packet, packetLen))
			return;
...
int main(int argc, char *argv[])
{
	...
	memset(&globalFilter, 0, sizeof(globalFilter));
