...
This command will print one line of MD5 digest for each file parameter
supplied.
...
int main(int argc, char *argv[])
{
	...
	unsigned char output[CRYPT_HASH_MD5_BYTES];
	int count1, count2;
	...
	for (count1 = 1; count1 < argc; count1 ++)
	{
		...
		do
		{
			...
			if (first)
			{
				status = cryptHashMd5(buffer, doneBytes, output,
			...
			{
				status = cryptHashMd5Cont(buffer, doneBytes, output,
					(sumBytes == 0) /* finalize? */, sumFileStream.size);
				if (status < 0)
				{
					errno = status;
					perror("cryptHashMd5Cont");
				}
			}
		...
		for (count2 = 0; count2 < CRYPT_HASH_MD5_BYTES; count2 ++)
