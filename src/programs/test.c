...
static int file_reopen(void)
{
	// To demonstrate a potential locking bug

	int status = 0;
	fileStream strm;
	char *fileName = "/temp/testfile";
	int count;
	...
	for (count = 0; count < 100; count ++)
	{
		...
		status = fileStreamOpen(fileName, (OPENMODE_CREATE |
			OPENMODE_TRUNCATE | OPENMODE_READWRITE), &strm);
		if (status < 0)
		{
			FAILMSG("Error %d creating file", status);
			goto out;
		}

		status = fileStreamWrite(&strm, sizeof(DATA), DATA);
		if (status < 0)
		{
			FAILMSG("Error %d writing file", status);
			goto out;
		}

		// Close the file
		status = fileStreamClose(&strm);
		if (status < 0)
		{
			FAILMSG("Error %d closing file", status);
			goto out;
		}
	}
