...
typedef struct {
	char name[8];					// 00 - 07 Filename
	char ext[3];					// 08 - 0A Extension
	unsigned char attrib;			// 0B - 0B Attributes
	unsigned char res;				// 0C - 0C Reserved
	unsigned char createTimeTenth;	// 0D - 0D Creation time (1/10 sec)
	unsigned short createTime;		// 0E - 0F Creation time
	unsigned short createDate;		// 10 - 11 Creation date
	unsigned short accessDate;		// 12 - 13 Access Data
	unsigned short firstClusterHi;	// 14 - 15 First cluster MSB
	unsigned short modTime;			// 16 - 17 Modification time
	unsigned short modDate;			// 18 - 19 Modification date
	unsigned short firstClusterLo;	// 1A - 1B First cluster LSB
	unsigned size;					// 1C - 1F File size
