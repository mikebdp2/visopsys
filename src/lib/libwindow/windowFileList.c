...
static typeIcon iconList[] = {
	...
	// the icon.  So, for example, if you want to make an icon for a type of
	// binary file, put it *before* the icon for plain binaries.
...
static int eventHandler(windowFileList *fileList, windowEvent *event)
{
	...
	// Check for events in our icon list.  We consider the icon 'clicked' if
	// it is a mouse click selection, or an ENTER key selection
