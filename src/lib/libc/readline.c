...
// a prompt.  If prompt is null, no prompt is issued.  The line returned is
// allocated with malloc(3), so the  caller must free it when finished.  The
// line returned has the final newline removed, so only the text of the line
// remains.
//
