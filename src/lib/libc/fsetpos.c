...
int fsetpos(FILE *theStream, fpos_t *pos)
{
	...
	// pointed to by theStream according to the value of the object pointed to
	// by pos, which must be a value obtained from an earlier call to
	// fgetpos() on the same stream.  Upon successful completion, fsetpos
	// returns 0.  Otherwise, -1 is returned and the global variable errno is
