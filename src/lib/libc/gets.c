...
char *gets(char *s)
{
	...
	// The algorithm is to run in a loop until a newline or EOF is
	// encountered.  Since we are reading from the text input stream, we need
	// to continually check whether there is any input before we call to get
	// the character.  If there is no input, yield the current time slice back
	// to the scheduler.
