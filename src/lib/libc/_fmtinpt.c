...
// This function does all the work of filling data values from input based on
// the format strings used by the scanf family of functions (and others, if
// desired)
...
int _fmtinpt(const char *input, const char *format, va_list list)
{
	...
	for (formatCount = 0; formatCount < formatLen; )
	{
		...
		// For other non-conversion specifiers, we simply verify that they
		// match and then skip them
