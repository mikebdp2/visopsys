...
int getopt(int argc, char *const argv[], const char *optstring)
{
	...
	or the text of the following argv-element, in optarg.  Two colons mean an
	option takes an optional arg; if there is text in the current argv-
	element, it is returned in optarg, otherwise optarg is set to zero.  This
	is a GNU extension.  If optstring contains W followed by a semicolon, then
	-W foo is treated as the long option --foo.  (The -W option is reserved by
	POSIX.2 for implementation extensions.)  This behaviour is a GNU
	extension, not available with libraries before GNU libc 2.
	...
	as soon as a non-option argument is encountered.  If the first character
	of optstring is '-', then each non-option argv-element is handled as if it
	...
	first character of optstring is a colon (':'), then getopt() returns ':'
