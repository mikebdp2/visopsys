...
// This function does all the work of expanding the format strings used by the
// printf family of functions (and others, if desired)
...
int _xpndfmt(char *output, int outputLen, const char *format, va_list list)
{
		...
		(outCount < (outputLen - 1))); )
	{
		...
		// Look for a zero digit, which indicates that any field width
		// argument is to be zero-padded
		zeroPad = 0;
		if (format[inCount] == '0')
		{
			zeroPad = 1;
		...
		// If it's a long long value, we need to get two 32-bit values and
		// combine them
		if (isLong)
		{
			...
			(format[inCount] == 'g') || (format[inCount] == 'G'))
		{
			...
			// to initialize the variable prematurely, above, causing an FPU
			// operation
		}
		else
		{
		...
		switch(format[inCount])
		{
			...
			default:
			{
				// Umm, we don't know what this is.  Just copy the '%' and the
				// format character to the output stream.
