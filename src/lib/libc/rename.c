...
int rename(const char *old, const char *new)
{
	...
	// points to the new pathname of the file.  Upon successful completion, 0
	// is returned.  Otherwise, -1 is returned and errno is set to indicate an
	// error.

	if (visopsys_in_kernel)
		return (errno = ERR_BUG);

	// Let the kernel do all the work
