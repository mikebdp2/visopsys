...
int fgetpos(FILE *theStream, fpos_t *pos)
{
	...
	// fsetpos() for repositioning the stream to its position at the time of
	// the call to fgetpos().  Upon successful completion, fgetpos() returns
	// 0.  Otherwise, it returns a non-zero value and sets errno to indicate
	// the error.
