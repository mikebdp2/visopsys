	...
	unsigned char *output, int finalize, unsigned totalBytes)
{
	...
	// around kernelCryptHashSha256Cont(), which will check parameters.

	int status = 0;
	unsigned start[CRYPT_HASH_SHA256_BYTES / 4];
	...
	start[5] = processorSwap32(H5);
	start[6] = processorSwap32(H6);
	start[7] = processorSwap32(H7);

	status = kernelCryptHashSha256Cont(message, len, (unsigned char *) start,
		finalize, totalBytes);
	if (status < 0)
		return (status);
	...
	unsigned char *output, int finalize, unsigned totalBytes)
{
	...
	int status = 0;
	unsigned hash[CRYPT_HASH_SHA256_BYTES / 4];
	unsigned chunk[16];
