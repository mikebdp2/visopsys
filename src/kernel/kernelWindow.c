...
int kernelWindowSetCharSet(kernelWindow *window, const char *charSet)
{
	...
	if (window->sysContainer && window->sysContainer->numComps &&
		(numComponents =
			window->sysContainer->numComps(window->sysContainer)))
	{
		array = kernelMalloc(numComponents * sizeof(kernelWindowComponent *));
	...
	if (window->mainContainer && window->mainContainer->numComps &&
		(numComponents =
			window->mainContainer->numComps(window->mainContainer)))
	{
		array = kernelMalloc(numComponents * sizeof(kernelWindowComponent *));
...
int kernelWindowSaveScreenShot(const char *name)
{
	...
	if (!name)
	{
		...
		if (fileName[strlen(fileName) - 1] != '/')
			strcat(fileName, "/");
		strcat(fileName, PATH_TEMP "/screenshot1.bmp");
	}
	else
	{
	...
	dialog = kernelWindowNew(kernelCurrentProcess->processId, "Screen shot");
	if (dialog)
	{
	...
	startTime = kernelCpuGetMs();

	// Try to save the screenshot image
...
out:
	...
	if (dialog)
	{
		// Try to make sure the dialog is shown for at least 2s

		currTime = kernelCpuGetMs();

		if ((currTime - startTime) < 2000)
			kernelMultitaskerWait(2000 - (currTime - startTime));
