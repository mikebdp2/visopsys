...
static spinLock freeEntriesLock;
...
{
	// This function is used to allocate more memory for the freeEntries list.
	// The freeEntriesLock should be held before calling this function.
...
{
	// This function will determine whether the supplied directory entry is a
	// 'leaf' directory.  A leaf directory is defined as a directory which
	// contains no other subdirectories with buffered contents.  Returns 1 if
	// the directory is a leaf, 0 otherwise.
...
{
	// This function is internal, and is called when the tree of file and
	// directory entries becomes too full.  It will "un-buffer" all of one
	// directory entry's contents (sub-entries) from memory.  The decision
	// about which entry to un-buffer is based on LRU (least recently used).
...
{
	// This will copy the applicable parts from a kernelFileEntry structure to
	// an external 'file' structure
	...
	fileStruct->blockSize =
		((kernelDisk *) entry->disk)->filesystem.blockSize;
}
...
static int dirIsEmpty(kernelFileEntry *entry)
{
	...
	// the non-entries '.' and '..'.  Returns 1 if the entry is a directory
	// and is empty, otherwise 0.
...
static int isDescendent(kernelFileEntry *leafEntry,
	kernelFileEntry *nodeEntry)
{
	...
	// If "nodeEntry" is not a directory, then it obviously has no
	// descendents.  Return false.
	...
	// Do a loop to step upward from the "leaf" through its parent
	// directories.  If a directory is its own parent (i.e. is the root
	// directory) we return false.  If a directory equals "nodeEntry", return
	// true.
	while (listEntry)
	{
...
static char *fixupPath(const char *originalPath)
{
	...
	for (count = 0; count < originalLength; count ++)
	{
		...
		{
			// We must determine whether this will be dot or dotdot.  If it is
			// dot, we simply remove this path level.  If it is dotdot, we
			// have to remove the previous path level.
	...
	while (1)
	{
		...
		while (listEntry)
		{
			...
			if ((int) strlen((char *) listEntry->name) == itemLength)
			{
				...
				// also.
				if (!strncmp((char *) listEntry->name, itemName,
						itemLength) ||
		...
		if (found)
		{
			...
			if (listEntry->type == linkT)
			{
				...
				if (!fsDisk)
				{
					kernelError(kernel_error, "Entry has a NULL disk "
						"pointer");
					return (listEntry = NULL);
				}
			}
			...
			if ((listEntry->type == dirT) && !listEntry->contents)
			{
				...
				// Increase the open count on the directory's entry while
				// we're reading it.  This will prevent the filesystem manager
...
static int fileCreate(const char *path)
{
	...
	// That's all but the last part of the "fileName" argument.  We have to
	// call this function to separate the two parts.
	status = kernelFileSeparateLast(path, prefix, name);
	if (status < 0)
		return (status);

	// Make sure "name" isn't empty.  This could happen if there WAS no last
	// item in the path to separate.
	if (!name[0])
	{
	...
	// OK, the directory exists.  Get the logical disk of the parent
	// directory.
	...
	// Set up the appropriate data items in the new entry that aren't done by
	// default in the kernelFileNewEntry() function
...
static int fileOpen(kernelFileEntry *entry, int openMode)
{
	...
	// Writing not allowed in read-only file system
	...
	if (openMode & OPENMODE_WRITE)
	{
		...
		if (openMode & OPENMODE_TRUNCATE)
		{
			...
			{
				kernelError(kernel_error, "The requested filesystem "
...
{
	// This is *somewhat* a wrapper function for the underlying function in
	// the filesystem's own driver, except it does do some general
	...
	// Make sure the item is really a file, and not a directory.  We have to
	// do different things for removing directory.
	if (entry->type != fileT)
	{
...
static int fileRemoveDir(kernelFileEntry *entry)
{
	...
	if (entry == rootEntry)
	{
		kernelError(kernel_error, "Cannot remove the root directory under "
			"any circumstances");
...
{
	// Calls the fileDelete and fileRemoveDir functions, recursively
	...
	if (entry->type == dirT)
	{
		...
		while (currEntry)
		{
			...
			while (currEntry && (!strcmp((char *) currEntry->name, ".") ||
...
static int fileCopy(file *sourceFile, file *destFile)
{
	...
	// (open for creation/writing) file.  Returns 0 on success, negative
	// otherwise.
	...
	while (srcBlocks)
	{
		...
		kernelDebug(debug_fs, "File write %u blocks to dest",
			destBlocksPerOp);
...
static int fileMove(kernelFileEntry *sourceEntry, kernelFileEntry *destDir)
{
	...
	// specified item is moved to that directory and will have the same name
	// as before.  If the target location is a file name, the item will be
	// moved and renamed.  Returns 0 on success, negtive otherwise.
	...
	if (status < 0)
	{
		// We were unable to place it in the new directory.  Better at least
		// try to put it back where it belongs in its old directory, with its
		// old name.  Whether this succeeds or fails, we need to try.
...
{
	// We're not initialized until the root directory has been set, below

	memset((void *) &freeEntriesLock, 0, sizeof(spinLock));
...
kernelFileEntry *kernelFileNewEntry(kernelDisk *fsDisk)
{
	...
	// Try to get a lock on the free entries list
	status = kernelLockGet(&freeEntriesLock);
	if (status < 0)
		return (entry = NULL);
	...
	if (!freeEntries)
	{
		...
		if (status < 0)
		{
			kernelLockRelease(&freeEntriesLock);
			return (entry = NULL);
		}
	}
...
void kernelFileReleaseEntry(kernelFileEntry *entry)
{
	...
	// Try to get a lock on the free entries list
	if (kernelLockGet(&freeEntriesLock) < 0)
		return;
...
int kernelFileInsertEntry(kernelFileEntry *entry, kernelFileEntry *dirEntry)
{
	...
	while (listEntry)
	{
		// We do a case-sensitive comparison here, regardless of whether the
		// filesystem driver cares about case.  We are worried about exact
		// matches.
		if (!strcmp((char *) listEntry->name, (char *) entry->name))
		{
			kernelError(kernel_error, "A file by the name \"%s\" already "
				"exists in the directory \"%s\"", entry->name,
				dirEntry->name);
			return (status = ERR_ALREADY);
		}
	...
	// Make sure that the number of entries in this directory has not exceeded
	// (and is not about to exceed) the maximum number of legal directory
	// entries
	...
	// 1. the filenames we now have (of the new entry and an existing entry in
	//   the list) are identical
	...
	// If the directory is empty, or we've reached the end of the list, we
	// should insert the new entry there
	if (!listEntry)
	{
		// Try to get a lock on the directory
		if (kernelLockGet(&dirEntry->lock) < 0)
		...
		kernelLockRelease(&dirEntry->lock);
	}

	// If the filenames are the same it's an error.  We are only worried about
	// exact matches, so do a case-sensitive comparison regardless of whether
	// the filesystem driver cares about case.
	...
	// Otherwise, listEntry points to an entry that should come AFTER our new
	// entry.  We insert our entry into the previous slot.  Watch out just in
	// case we're BECOMING the first item in the list!
	else
	{
		// Try to get a lock on the directory
		if (kernelLockGet(&dirEntry->lock) < 0)
...
{
	// This function is used to delete an entry from its parent directory.  It
	// DOES NOT deallocate the file entry structure itself.  That must be
	// done, if applicable, by the calling function.
	...
	// Try to get a lock on the directory
	if (kernelLockGet(&parentEntry->lock) < 0)
		return (status = ERR_NOLOCK);
	...
	// If we're deleting the first file of the directory, we have to change
	// the parent directory's "first file" pointer (in addition to the other
	// things we do) to point to the following file, if any
	if (entry == parentEntry->contents)
		parentEntry->contents = nextEntry;

	// Make this item's 'previous' item refer to this item's 'next' as its own
	// 'next'
	if (previousEntry)
	...
	kernelLockRelease(&parentEntry->lock);
...
int kernelFileMakeDotDirs(kernelFileEntry *parentEntry,
...
int kernelFileUnbufferRecursive(kernelFileEntry *entry)
{
	...
	// Mostly, this will be useful only when unmounting filesystems. Returns 0
	...
	// currently have a valid write lock?
	listEntry = entry->contents;
	while (listEntry)
	{
	...
	if (listEntry)
	{
		// There are files open for writing here
...
int kernelFileEntrySetSize(kernelFileEntry *entry, unsigned newSize)
{
	...
	if (entry->blocks != newBlocks)
	{
		...
		{
			kernelError(kernel_error, "Filesystem driver for %s cannot "
				"change the number of blocks", entry->name);
			return (status = ERR_NOSUCHFUNCTION);
		}
	}
...
{
	// This function is a user-accessible wrapper for our internal fixupPath()
	// funtion, above
	...
	char *fileName)
{
	...
	// a(nother) '/' or '\' character.  Of course, that might be the '/' or
	// '\' of root directory fame.  We know we'll hit at least one.
	...
	// Make sure neither the path or the combined exceed the maximum lengths
...
/////////////////////////////////////////////////////////////////////////
//
//  Below here, the functions are largely external wrappers for functions
//  above, which are exported outside the kernel and accept text strings for
//  file names, etc.
//
/////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////
...
int kernelFileNext(const char *path, file *fileStruct)
{
	...
	// Find the previously accessed file in the current directory.  Start with
	// the first entry.
	if (entry->contents)
	{
		listEntry = entry->contents;
...
int kernelFileClose(file *fileStruct)
{
	...
	// If the file was write-locked, unlock it
	kernelLockRelease(&entry->lock);
...
int kernelFileRead(file *fileStruct, unsigned blockNum, unsigned blocks,
	void *fileBuffer)
{
	// This function is responsible for all reading of files.  It takes a file
	// structure of an opened file, the number of blocks to skip before
	// commencing the read, the number of blocks to read, and a buffer to use.
	// Returns negative on error, otherwise it returns the same status as the
	// driver function.
	...
	// Check on the filesystem driver function we want to call
	...
	// Return the same status that the driver function returned
	return (status);
}
...
int kernelFileWrite(file *fileStruct, unsigned blockNum, unsigned blocks,
	void *fileBuffer)
{
	// This function is responsible for all writing of files.  It takes a file
	// structure of an opened file, the number of blocks to skip before
	// commencing the write, the number of blocks to write, and a buffer to
	// use.  Returns negative on error, otherwise it returns the same status
	// as the driver function.
...
int kernelFileDelete(const char *fileName)
{
	...
	// The file exists.  Call our internal 'delete' function.
...
int kernelFileDeleteSecure(const char *fileName, int passes)
{
	...
	if (entry->type != fileT)
	{
		kernelError(kernel_error, "Item to delete (%s) is not a file",
			entry->name);
		return (status = ERR_NOTAFILE);
	}
	...
	// Get a buffer as big as all of the blocks allocated to the file

	// TODO: Guess that means this function will only work for files that can
	// fit into memory!
...
int kernelFileCopy(const char *srcName, const char *destName)
{
	...
	// 'srcPath', to either the directory or file name pointed to by
	// 'destPath'
	...
	// Determine whether the destination file exists, and if so whether it is
	// a directory
	...
	if (entry)
	{
		...
		if (entry->type == dirT)
		{
			...
			// destination file in that directory that shares the same
			// filename as the source.  Construct the new name.
	...
	// Make sure the destination file's block size isn't zero (this would lead
	// to a divide-by-zero error at writing time)
	...
	if (status >= 0)
	{
		// Set the size of the destination file so that it matches that of the
		// source file (as opposed to a multiple of the block size and the
		// number of blocks it consumes)
...
int kernelFileCopyRecursive(const char *srcPath, const char *destPath)
{
	...
	// Determine whether the source item exists, and if so whether it is a
	// directory
...
int kernelFileMove(const char *srcName, const char *destName)
{
	...
	// Check whether the destination file exists.  If it exists and it is a
	// file, we need to delete it first.
	...
	if (destEntry)
	{
		...
		if (destEntry)
		{
			// There already exists an item at that location, and it is not a
			// directory.  It needs to go away.  Really, we should keep it
	...
	{
		// No such file exists.  We need to get the destination directory from
		// the destination path we were supplied.
...
int kernelFileTimestamp(const char *path)
{
	...
	// Now we change the internal data fields of the file's data structure to
	// match the current date and time (don't touch the creation date/time)
...
int kernelFileSetSize(file *fileStruct, unsigned newSize)
{
	...
	if (!fileStruct->handle)
	{
		kernelError(kernel_error, "NULL file handle for set size.  Not "
			"opened first?");
		return (status = ERR_NULLPARAMETER);
	}
	...
	if (!(fileStruct->openMode & OPENMODE_WRITE))
	{
		kernelError(kernel_error, "File %s has not been opened for writing "
			"%x", fileStruct->name, fileStruct->openMode);
		return (status = ERR_INVALID);
	}
