...
static inline void debugGroupDesc(extGroupDesc32 *groupDesc)
{
...
static inline unsigned long long getSectorNumber(extInternalData *extData,
...
static extInternalData *getExtData(kernelDisk *theDisk)
{
	...
	if (extData->superblock.feature_incompat & EXT_INCOMPAT_64BIT)
	{
		kernelDebug(debug_fs, "EXT is 64-bit");
		extData->bits64 = 1;
	}
	...
	totalBlocks = extData->superblock.blocks_count;
	if (extData->bits64)
	{
		totalBlocks |= ((unsigned long long)
			extData->superblock.blocks_count_hi << 32);
	}

	// Calculate the number of block groups
	extData->numGroups = (unsigned)((totalBlocks +
		(extData->superblock.blocks_per_group - 1)) /
			extData->superblock.blocks_per_group);

	extData->groupDescSize = sizeof(extGroupDesc32);
	if (extData->bits64 && (extData->superblock.desc_size > 32))
	{
		extData->groupDescSize = extData->superblock.desc_size;

		if (extData->groupDescSize != sizeof(extGroupDesc64))
		{
			kernelError(kernel_error, "EXT group descriptor size %u is "
				"unsupported", extData->groupDescSize);
			kernelFree((void *) extData);
			return (extData = NULL);
		}
	}
	...
	groupDescBlocks = (((extData->numGroups * extData->groupDescSize) +
	...
	extInode *inode)
{
	...
	extGroupDesc32 *group32 = NULL;
	extGroupDesc64 *group64 = NULL;
	unsigned long long inodeTable = 0;
	...
	if (extData->groupDescSize == sizeof(extGroupDesc64))
	{
		group64 = &((extGroupDesc64 *) extData->groups)[groupNumber];
		debugGroupDesc((extGroupDesc32 *) group64);
		inodeTable = (((unsigned long long) group64->inode_table_hi << 32) |
			group64->g32.inode_table);
	}
	else
	{
		group32 = &((extGroupDesc32 *) extData->groups)[groupNumber];
		debugGroupDesc(group32);
		inodeTable = group32->inode_table;
	}

	kernelDebug(debug_fs, "EXT read inode table block %llu, sector %llu",
		(inodeTable + inodeTableBlock), getSectorNumber(extData, (inodeTable +
		inodeTableBlock)));

	// Read the applicable inode table block
	status = kernelDiskReadSectors((char *) extData->disk->name,
		getSectorNumber(extData, (inodeTable + inodeTableBlock)),
	...
	unsigned startBlock, unsigned numBlocks, void *buffer)
{
	...
	return (readExtentNode(extData, (extExtent *) &inode->u.extent,
		startBlock, numBlocks, &doneBlocks, buffer));
}
	...
	int longFormat __attribute__((unused)), progress *prog)
{
	...
	extGroupDesc32 *groupDescs = NULL;
	...
	groupDescBlocks = (((blockGroups * sizeof(extGroupDesc32)) +
...
static int mount(kernelDisk *theDisk)
{
	...
	incompatFlags &= ~(EXT_INCOMPAT_FLEXBG | EXT_INCOMPAT_64BIT |
		EXT_INCOMPAT_EXTENTS | EXT_INCOMPAT_RECOVER | EXT_INCOMPAT_FILETYPE);
