...
// Global data required by the scheduler
static volatile struct {
	int stop;
	unsigned timeSlices;
	unsigned systemTime;
	unsigned schedulerTime;
	unsigned sliceCount;

} schedData = { 0 };

// Global data required by the exception handler
static volatile struct {
	int number;
...
static int createProcessContext(kernelProcess *proc)
{
	...
	status = kernelDescriptorSet(
		...
		1,								// Present in memory
		PRIVILEGE_SUPERVISOR,			// TSSes are supervisor priv level
		0,								// TSSes are system segs
		0xB,							// TSS, 32-bit, busy
...
static void exceptionHandler(void)
{
	...
	while (1)
	{
		...
		if (multitaskingEnabled)
		{
			...
			{
				exception.process->state = proc_stopped;
			}
		}

		if (!multitaskingEnabled || (exception.process == kernelProc))
		{
			sprintf(message, "The kernel has experienced");
		}
		else
		{
			sprintf(message, "Process \"%s\" caused",
				exception.process->name);
		}

		sprintf((message + strlen(message)), " %s %s exception",
			exceptionVector[exception.number].a,
			exceptionVector[exception.number].name);

		if (exception.address >= KERNEL_VIRTUAL_ADDRESS)
			kernOrApp = "kernel";
		else
		...
		if (multitaskingEnabled)
		{
			...
			symbolName = kernelLookupClosestSymbol(exception.process, (void *)
				exception.address);
		}
		...
		if (symbolName)
		{
			sprintf((message + strlen(message)), " in %s function %s (%08x)",
				kernOrApp, symbolName, exception.address);
		}
		else
		{
			sprintf((message + strlen(message)), " at %s address %08x",
				kernOrApp, exception.address);
		}
		...
		{
			// Get process info
			debugContext(exception.process, details, MAXSTRINGLENGTH);

			// Try a stack trace
			kernelStackTrace(exception.process, (details + strlen(details)),
		...
		// itself)
		if (kernelGraphicsAreEnabled() && strcmp((char *)
			exception.process->name, ERRORDIALOG_THREADNAME))
		{
			kernelErrorDialog("Application Exception", message, details);
		}

		// The collector thread may now dismantle the process
		exception.process->state = proc_finished;
...
static int spawnExceptionThread(void)
{
	...
	// Set the thread state to sleeping
...
static void collectorThread(void)
{
	// This is the collector thread.  It runs in a loop looking for processes
	// who have been marked as finished, and dismantles them.
	...
	while (1)
	{
		...
		while (proc)
		{
			...
			{
				// Dismantle the process, and remove it from the list
				if (kernelMultitaskerKillProcess(proc->processId) < 0)
				{
					// We couldn't kill it, so we will mark it as a zombie
					// process
...
static int spawnCollectorThread(void)
{
	// This function will create the collector thread at initialization time.
	// Returns 0 on success, negative otherwise.

	int status = 0;

	// The collector thread needs to be a child of the kernel
	status = kernelMultitaskerSpawn(&collectorThread, "collector thread",
...
static int spawnIdleThread(void)
{
	...
	procId = kernelMultitaskerSpawn(&idleThread, "idle thread",
		0 /* no args */, NULL /* no args */, 1 /* run */);
	if (procId < 0)
		return (procId);

	idleProc = getProcessById(procId);
	if (!idleProc)
		return (status = ERR_NOSUCHPROCESS);

	// Set it to the lowest priority
	status = kernelMultitaskerSetProcessPriority(procId,
		(PRIORITY_LEVELS - 1));
	if (status < 0)
	{
...
static void contextSwitch(kernelProcess *nextProc)
{
	// Check params
	if (!nextProc)
		return;

	// Mark the next process as not busy
	markProcessBusy(nextProc, 0);

	// Export (to the rest of the multitasker) the pointer to the selected
	// process
...
static int schedulerShutdown(void)
{
	...
	// scheduler and return control to the kernel's main task.  This will
	...
	// schedData.stop to a nonzero value.  The scheduler will then invoke this
	// function when it's ready.

	// Interrupts must be suspended/disabled when this function is called
...
static kernelProcess *chooseNextProcess(void)
{
	...
	while (miscProc)
	{
		if (miscProc->state == proc_waiting)
		{
			...
			// Get the CPU time
			if (miscProc->waitUntil && !cpuTime)
				cpuTime = kernelCpuGetMs();

			// If the process is waiting for a specified time.  Has the
			// requested time come?
			if (miscProc->waitUntil && (miscProc->waitUntil < cpuTime))
			{
		...
		else if (miscProc->switchedByCall && (miscProc->lastSlice ==
			schedData.timeSlices))
		{
...
{
	// This is the kernel multitasker's scheduler.  This hands out time slices
	// to all processes, including the kernel itself, initially, before it
	// goes to sleep.
	...
	// This will be changed by a context switch, thus 'volatile'
	...
	// Were we invoked by an interrupt, or a call?
	if (kernelCurrentProcess->switchedByCall)
	...
	// actually expired, or whether we were called for some other reason (for
	// example a yield()).
	...
	schedData.systemTime += timeUsed;

	// Have we had the equivalent of a full timer revolution?  If so, we need
	// to call the standard timer interrupt handler.
	if (schedData.systemTime >= SYSTIMER_FULLCOUNT)
	{
		// Reset to zero
		schedData.systemTime = 0;
	...
	schedData.schedulerTime += timeUsed;
	sliceCount = (schedData.schedulerTime / TIME_SLICE_LENGTH);
	if (sliceCount > schedData.sliceCount)
	{
		// Increment the count of time slices.  This can just keep going up
		// until it wraps, which is no problem.
		schedData.timeSlices += 1;
		schedData.sliceCount = sliceCount;
	}

	if (kernelCurrentProcess->state == proc_running)
	{
		// Change the state of the previous process to ready, since it was
		// interrupted while still on the CPU
		kernelCurrentProcess->state = proc_ready;
	}

	// Add the last timeslice to the process's CPU time
	kernelCurrentProcess->cpuTime += timeUsed;

	// Record the current timeslice number, so we can remember when this
	// process was last active (see chooseNextProcess())
	kernelCurrentProcess->lastSlice = schedData.timeSlices;

	// Every CPU_PERCENT_TIMESLICES timeslices we will update the %CPU value
	// for each process currently in the list
	if (sliceCount >= CPU_PERCENT_TIMESLICES)
	{
		...
		schedData.schedulerTime = schedData.sliceCount = 0;
	}

	if (exception.number)
	{
		// If we were processing an exception (either the exception thread or
	...
	while (kernelSysTimerSetupTimer(0 /* timer */, 0 /* mode */,
		TIME_SLICE_LENGTH) < 0)
	{
		kernelError(kernel_warn, "The scheduler was unable to control the "
			"system timer");
	}
	...
	if (nextProc != kernelCurrentProcess)
		contextSwitch(nextProc);

	// Consider kernelCurrentProcess and any values on the stack to be from
	// the new process (and previous switch) from this point onward

out:
	if (kernelCurrentProcess->switchedByCall)
	{
		kernelCurrentProcess->switchedByCall = 0;
		processorRestoreInts(interrupts);
	}
}


static void schedulerInterrupt(void)
{
	// This is the system timer interrupt handler that invokes the scheduler
...
static int schedulerInitialize(void)
{
	...
	// Install a handler for the scheduler's timer interrupt.  After this
	// point, our scheduler will run with every clock tick.
	status = kernelInterruptHook(INTERRUPT_NUM_SYSTIMER, &schedulerInterrupt);
	if (status < 0)
	{
		processorRestoreInts(interrupts);
		return (status);
...
int kernelMultitaskerInitialize(void *kernelStack, unsigned kernelStackSize)
{
	...
	// Create a "collector" thread to clean up finished processes
	status = spawnCollectorThread();
	if (status < 0)
		return (status);
...
int kernelMultitaskerShutdown(int nice)
{
	...
	// Set the scheduler stop flag to stop the scheduler
	schedData.stop = 1;
...
void kernelException(int num, unsigned address)
{
	...
	if (exception.number)
	{
		kernelPanic("Double-fault (%s) while processing %s %s exception",
			exceptionVector[num].name, exceptionVector[exception.number].a,
			exceptionVector[exception.number].name);
	}

	exception.number = num;
	exception.process = kernelCurrentProcess;
	exception.address = address;

	// If there's a handler for this exception type, call it
	if (exceptionVector[exception.number].handler &&
		(exceptionVector[exception.number].handler() >= 0))
	{
	...
	// If multitasking is enabled, switch to the exception handler thread.
	// Otherwise, just call the exception handler as a function.
	if (multitaskingEnabled)
		contextSwitch(exceptionProc);
	else
		exceptionHandler();
...
void kernelMultitaskerYield(void)
{
	...
	// We accomplish a yield by doing a call to the scheduler.  The scheduler
	// sees this almost as if the current timeslice had expired.
	kernelCurrentProcess->switchedByCall = 1;
	scheduler();
}
...
int kernelMultitaskerKillAll(void)
{
	...
	// Stop all processes, except the kernel process, exception thread, idle
	// thread, and the current process
