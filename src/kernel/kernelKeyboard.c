...
static void screenshotThread(void)
{
	...
	do
	{
		sprintf(fileName, PATH_TEMP "/screenshot%d.bmp", count++);
...
int kernelKeyboardInitialize(void)
{
	...
	// keyboard mapping.  Any keyboard driver should call this at the end of
	// successful device detection, to ensure that we will be able to process
	// their inputs.
...
{
	// Called by a device driver (PS2, USB, etc) to add a keyboard to the
	// list.  We allow this before we've been initialized, above, because
	// hardware drivers do their initial detection first.
...
int kernelKeyboardSetStream(stream *newStream)
{
	...
	// Save the address of the kernelStream we were passed to use for keyboard
	// data
