...
static int readPrimaryVolDesc(isoInternalData *isoData)
{
	...
	// Do a dummy read from the CD-ROM to ensure that the TOC has been
	// properly read, and therefore the information for the last session is
	// available
	status = kernelDiskReadSectors((char *) isoData->disk->name,
		ISO_PRIMARY_VOLDESC_SECTOR, 1, (isoPrimaryDescriptor *)
		&isoData->volDesc);
	if (status < 0)
		return (status);
	...
	// Not sure why this is necessary, exactly, since it's supposed to be a
	// 4-byte value.  But it's not.
...
static void makeSystemTime(unsigned char *isoTime, unsigned *date,
	unsigned *theTime)
{
	// This function takes an ISO date/time value and returns the equivalent
	// in packed-BCD system format
...
{
	// Reads a directory record from the on-disk structure in 'buffer' to our
	// structure
	...
	memcpy((void *) &fileData->dirRec, record, record->fixed.recordLength);

	// Copy the name into the file entry
	memcpy((char *) fileEntry->name, (char *) fileData->dirRec.name,
		fileData->dirRec.fixed.nameLength);
	fileEntry->name[fileData->dirRec.fixed.nameLength] = '\0';

	// Find the semicolon (if any) at the end of the name
	for (count = (fileData->dirRec.fixed.nameLength - 1); count > 0; count --)
	{
	...
	if (fileData->dirRec.fixed.flags & ISO_FLAGMASK_DIRECTORY)
		fileEntry->type = dirT;

	if (fileData->dirRec.fixed.flags & ISO_FLAGMASK_ASSOCIATED)
		fileEntry->type = linkT;

	// Get the date and time
	makeSystemTime((unsigned char *) fileData->dirRec.fixed.date,
	...
	fileEntry->size = fileData->dirRec.fixed.size;
...
static isoInternalData *getIsoData(kernelDisk *theDisk)
{
	...
	readDirRecord((isoDirectoryRecord *)
		&isoData->volDesc.rootDirectoryRecord,
	...
	strncpy((char *) theDisk->filesystem.label, (char *)
		isoData->volDesc.volumeIdentifier, 32);
...
static int scanDirectory(isoInternalData *isoData, kernelFileEntry *dirEntry)
{
	...
	if (!buffer)
	{
		kernelError(kernel_error, "Unable to get memory for directory "
			"buffer");
		return (status = ERR_MEMORY);
	}

	status = kernelDiskReadSectors((char *) isoData->disk->name,
		scanDirRec->dirRec.fixed.blockNumber, dirEntry->blocks, buffer);
	if (status < 0)
	{
		kernelFree(buffer);
		return (status);
	...
	while (ptr < (buffer + bufferSize))
	{
		...
		{
			// This is a NULL entry.  If the next entry doesn't fit within the
			// same logical sector, it is placed in the next one.  Thus, if we
			// are not within the last sector we read, skip to the next one.
...
static int newEntry(kernelFileEntry *entry)
{
	...
	if (!entry->driverData)
	{
		kernelError(kernel_error, "Error allocating memory for ISO directory "
			"record");
		return (status = ERR_MEMORY);
	}
...
static int resolveLink(kernelFileEntry *linkEntry)
{
	...
	// resolved.  By default this driver never resolves ISO symbolic links
	...
	unsigned blocks, unsigned char *buffer)
{
	...
	status = kernelDiskReadSectors((char *) isoData->disk->name,
		(dirRec->dirRec.fixed.blockNumber + blockNum), blocks, buffer);
...
{
	// This function receives an empty file entry structure, which represents
