...
static kernelFilesystemDriver *getDriver(const char *name)
{
	...
	for (count = 0; count < driverCounter; count ++)
	{
		if (!strncasecmp(name, driverArray[count]->driverTypeName,
			strlen(name)))
		{
			return (driverArray[count]);
		}
...
static kernelFilesystemDriver *detectType(kernelDisk *theDisk)
{
	...
	// moment there will be a set number of known filesystem types that will
	// be more-or-less hard-coded into this function.  Of course this isn't
	// desirable and should/will be fixed to be more flexible in the future.
	// The function returns an enumeration value reflecting the type it found
	// (including possibly "unknown").
	...
	// We will assume that the detection functions being called will do all of
	// the necessary checking of the kernelDisk before using it.  Since we're
	// not actually using it here, we won't duplicate that work.
...
static int unmount(const char *path, int removed)
{
	...
	// Starting at the mount point, unbuffer all files from the file entry
	// tree
	...
	// If the the device is still present, call the filesystem driver's
	// unmount function
	...
	if (!removed)
	{
		...
		if (status < 0)
		{
			kernelError(kernel_warn, "Unable to sync disk \"%s\" after "
				"unmount", theDisk->name);
		}
	...
	const char *label, int longFormat, progress *prog)
{
	...
	{
		kernelError(kernel_error, "The filesystem driver does not support "
...
int kernelFilesystemCheck(const char *diskName, int force, int repair,
	progress *prog)
{
	// This function is a wrapper for the filesystem driver's 'check'
	// function, if applicable
...
int kernelFilesystemResizeConstraints(const char *diskName,
	uquad_t *minBlocks, uquad_t *maxBlocks, progress *prog)
{
	...
	{
		kernelError(kernel_error, "The filesystem driver does not support "
			"the 'resize constraints' operation");
		return (status = ERR_NOSUCHFUNCTION);
	}
...
{
	// This function creates and registers (mounts) a new filesystem
	// definition.  If successful, it returns the filesystem number of the
	// filesystem it mounted.  Otherwise it returns negative.
	...
	// Is it the root filesystem?
	isRoot = !strcmp(mountPoint, "/");

	// If this is NOT the root filesystem we're mounting, we need to make sure
	// that the mount point doesn't already exist.  This is because the root
	// directory of the new filesystem will be inserted into its parent
	// directory here.  This is un-UNIXy.
	...
	else
	{
		...
		// Insert the filesystem's root directory into the file entry tree
...
{
	// This is merely a wrapper function for the equivalent function in the
	// requested filesystem's own driver.  It takes nearly-identical arguments
	// and returns the same status as the driver function.
	...
	if (!theDisk)
	{
		kernelError(kernel_error, "No disk for mount point \"%s\"",
			mountPoint);
	...
	// OK, we just have to check on the filsystem driver function we want to
	// call
...
{
	// This function simply returns the block size of the filesystem that
	...
	return (blockSize = ((kernelDisk *)
		fileEntry->disk)->filesystem.blockSize);
}
