...
// commonly found on DOS and Windows disks
...
static int readVolumeInfo(fatInternalData *fatData)
{
	...
	// Also, the bytes-per-sector field should match the value for the disk
	// we're using
	...
	// Hmm, I'm not too sure what to make of the hidden-sectors field. The way
	// I read the documentation is that it describes the number of sectors in
	// any physical partitions that preceed this one.  However, we already get
	// this value from the master boot record where needed. We will ignore the
	// value of this field.
	...
	if (!fatData->totalSects)
	{
		// This combination is not legal for FAT
	...
	// Now we must determine which type of filesystem we've got here.  We must
	// do a few calculations to finish gathering the information we need to
	...
	if (fatData->bpb.rootEntCount)
	{
		...
		fatData->rootDirSects = (((fatData->bpb.rootEntCount *
			sizeof(fatDirEntry)) + (fatData->bpb.bytesPerSect - 1)) /
	...
	// We have already ensured that the sectors-per-cluster value is non-zero
	// (don't worry about a divide-by-zero error here)
	...
	// Now we now have enough data to determine the type of this FAT
	...
	if (fatData->fsType == fat32)
	{
		...
		// The next value is the "extended flags" field for FAT32.  This value
		// indicates the number of "active" FATS.  Active FATS are significant
		// when FAT mirroring is active.  While our filesystem driver does not
		// do runtime mirroring per se, it does update all FATs whenever the
		// master FAT is changed on disk.  Thus, at least on disk, all FATs
		// are generally synchronized.  We will ignore the field for now.

		// We have already read the FAT32 sectors-per-fat value.  This is
		// necessitated by a "logic bug" in the filesystem specification.  See
		// the rather lengthy explanation nearer the beginning of this
		// function.

		// Now we have the FAT32 version field.  The FAT32 version we are
		// supporting here is 0.0.  We will not mount a FAT volume that has a
		// higher version number than that (but we don't need to save the
		...
		// The starting cluster number of the root directory must be >= 2, and
		// <= (data-clusters + 1)
...
static int writeVolumeInfo(fatInternalData *fatData)
{
	...
	// If FAT32 and backupBootF32 is non-zero, make a backup copy of the boot
	// sector
...
static unsigned *readFatSectors(fatInternalData *fatData, unsigned sector,
	unsigned numSectors)
{
	// Allocates memory for the requested FAT sector(s) and reads them

	int status = 0;
	unsigned *fatSects = NULL;

	fatSects = kernelMalloc(fatData->disk->physical->sectorSize * numSectors);
	if (!fatSects)
		return (fatSects);

	status = kernelDiskReadSectors((char *) fatData->disk->name,
		(fatData->bpb.rsvdSectCount + sector), numSectors, fatSects);
	if (status < 0)
	{
		kernelFree(fatSects);
		return (fatSects = NULL);
	}
...
static int writeFatSectors(fatInternalData *fatData, unsigned sector,
	unsigned numSectors, unsigned *fatSects)
{
...
{
	// Given a range of FAT entries to read, return them in the supplied array
	...
	{
		kernelError(kernel_error, "Requested FAT range (%u->%u) is beyond "
			"the limits of the table (%u)", firstEntry, lastEntry,
	...
	{
		case fat12:
			// FAT 12 entries are 3 nybbles each.  Take the entry numbers we
			// were given and multiply them by 3/2 to get the byte offsets.
			firstOffset = (firstEntry + (firstEntry >> 1));
			lastOffset = (lastEntry + (lastEntry >> 1));
			break;

		case fat16:
			// FAT 16 entries are 2 bytes each.  Take the entry numbers we
			// were given and multiply them by 2 to get the byte offsets.
			firstOffset = (firstEntry * 2);
			lastOffset = (lastEntry * 2);
			break;

		case fat32:
			// FAT 32 entries are 4 bytes each.  Take the entry numbers we
			// were given and multiply them by 4 to get the byte offsets.
	...
	for (count = 0; count < numEntries; count ++)
	{
		...
		{
			case fat12:
				// FAT 12 entries are 3 nybbles each.  entryOffset is the
				// index of the WORD value that contains the value we're
				// looking for.
				entries[count] = *((unsigned short *)((void *) fatSects +
					entryOffset));
			...
			case fat32:
				...
				entries[count] = (*((unsigned *)((void *) fatSects +
					entryOffset)) & 0x0FFFFFFF);
				entryOffset += 4;
				break;
	...
	unsigned value)
{
	...
	{
		case fat12:
			// FAT 12 entries are 3 nybbles each.  Take the entry number we
			// were given and multiply it by 3/2 to get the starting byte.
		...
		case fat16:
			// FAT 16 entries are 2 bytes each.  Take the entry number we were
			// given and multiply it by 2 to get the starting byte.
			entryOffset = (entryNumber * 2);
			break;

		case fat32:
			// FAT 32 entries are 4 bytes each.  Take the entry number we were
			// given and multiply it by 4 to get the starting byte.
	...
	switch (fatData->fsType)
	{
		case fat12:
			...
			entryValue = *((short *)((void *) fatSects + entryOffset));
			if (entryNumber % 2)
			{
				entryValue &= 0x000F;
			...
			*((short *)((void *) fatSects + entryOffset)) = entryValue;
			break;

		case fat16:
			// FAT 16 entries are 2 bytes each
			*((short *)((void *) fatSects + entryOffset)) = value;
			break;

		case fat32:
			// FAT 32 entries are 4 bytes each
			oldValue = *((unsigned *)((void *) fatSects + entryOffset));
			// Make sure we preserve the top 4 bits of the previous entry
			entryValue = (value | (oldValue & 0xF0000000));
			*((unsigned *)((void *) fatSects + entryOffset)) = entryValue;
			break;
	...
	status = writeFatSectors(fatData, fatStartSector, numFatSectors,
		fatSects);
	...
	unsigned *clusters)
{
	...
	*clusters = 0;
	...
	// total each time.  A value of terminalClust or more means that there are
	// no more sectors.
	while (1)
	{
		if ((currentCluster < 2) || (currentCluster >=
...
{
	// This function returns an unused cluster or sequence of clusters back to
	// the free list, and marks them as unused in the volume's FAT table.
	...
	unsigned *startCluster)
{
	...
	*startCluster = 0;
	...
	// Move through the free cluster bitmap, looking for the first free chunk
	// that is big enough to accommodate the request.  We also keep track of
	// the biggest (but not big enough) block that we have encountered so far.
	// This enables us to select the next-biggest available block if no single
	// block is large enough to accommodate the whole request.
	...
	for (count = 2; count < terminate; count ++)
	{
		...
		// search.  If this iteration of "count" is divisible by 8, then we
		// can let the processor scan the whole byte ahead looking for ANY
		// unused clusters.
	...
	unsigned *lastCluster)
{
	// This function is internal, and takes as a parameter the starting
	// cluster of a file/directory.  The function will traverse the FAT table
	// entries belonging to the file/directory, and return the number of the
	// last cluster used by that item.
	...
	// Now we go through a loop to step through the cluster numbers.  A value
	// of terminalClust or more means that there are no more clusters.
	while (1)
	{
		if ((currentCluster < 2) || (currentCluster >=
			fatData->terminalClust))
	...
	unsigned *nthCluster)
{
	...
	// cluster of a file/directory.  The function will traverse the FAT table
	// entries belonging to the file/directory, and return the number of the
	// requested cluster used by that item.  Zero-based.
	...
	unsigned skipClusters, unsigned readClusters, unsigned char *buffer)
{
	...
	// Now, it's possible that the file actually contains fewer clusters than
	// the 'readClusters' value.  If so, replace our readClusters value with
	// that value.
	...
	{
		// At the start of this loop, we know the current cluster.  If this is
		// not the last cluster we're reading, peek at the next one.
...
static int scanDirectory(fatInternalData *fatData, kernelDisk *theDisk,
	kernelFileEntry *currentDir, unsigned char *dirBuffer,
	unsigned dirBufferSize)
{
	...
	int status = 0;
	fatDirEntry *dirEntry = (fatDirEntry *) dirBuffer;
	...
	for (count1 = 0; count1 < (dirBufferSize / sizeof(fatDirEntry));
		count1 ++)
	{
		// Now we must determine whether this is a valid, undeleted file

		if ((unsigned char) dirEntry[count1].name[0] == 0xE5)
		{
			// E5 means this is a deleted entry
			continue;
		}
		else if (dirEntry[count1].name[0] == 0x05)
		{
			// 05 means that the first character is REALLY E5
			dirEntry[count1].name[0] = 0xE5;
		}
		else if (!dirEntry[count1].name[0])
		{
			// 00 means that there are no more entries
			break;
		}
		else if (dirEntry[count1].attrib == 0x0F)
		{
			// It's a long filename entry.  Skip it until we get to the
			// regular entry.
			continue;
		}
		else if (!strncmp(dirEntry[count1].name, ".          ",
			FAT_8_3_NAME_LEN) || !strncmp(dirEntry[count1].name,
			"..         ", FAT_8_3_NAME_LEN))
		{
			// Skip '.' and '..' entries
			continue;
		...
		// type of the file.
		if (dirEntry[count1].attrib & FAT_ATTRIB_VOLUMELABEL)
		{
			...
			{
				memcpy((unsigned char *) fatData->rootDirLabel,
					(unsigned char *) &dirEntry[count1], sizeof(fatDirEntry));
				setVolumeLabel(theDisk, (char *) &dirEntry[count1]);
			}
		...
		// Get the entry data structure.  This should have been created by a
		// call to our NewEntry function by the kernelFileNewEntry call.
		...
		subEntry = ((void *) &dirEntry[count1 - 1]);

		if ((count1 > 0) && (((fatDirEntry *) subEntry)->attrib == 0x0F))
		{
			longFilename = 1;
			longFilenamePos = 0;
			...
			while (1)
			{
				...
				// this file.  If not, we look at the previous entry from
				// subEntry and loop.
				if (subEntry[0] & 0x40)
					break;
				else
					subEntry -= sizeof(fatDirEntry);
			}
		...
		strncpy((char *) entryData->shortAlias, dirEntry[count1].name,
			FAT_8_3_NAME_LEN);
		...
		// drain-bamaged format used by DOS.
		if (!longFilename)
		{
			strncpy((char *) newItem->name, (char *) entryData->shortAlias, 8);
			newItem->name[8] = '\0';

			// Insert a NULL if there's a [space] character anywhere in this
			// part
			...
			// Insert a NULL if there's a [space] character anywhere in the
			// name
			for (count2 = 0; count2 < strlen((char *) newItem->name);
				count2 ++)
			{
		...
		entryData->attributes = (unsigned) dirEntry[count1].attrib;
		...
		// reserved
		entryData->res = (unsigned) dirEntry[count1].res;

		// timeTenth
		entryData->timeTenth = (unsigned) dirEntry[count1].createTimeTenth;

		// Creation time
		newItem->creationTime = makeSystemTime(dirEntry[count1].createTime);

		// Creation date
		newItem->creationDate = makeSystemDate(dirEntry[count1].createDate);

		// Last access date
		newItem->accessedDate = makeSystemDate(dirEntry[count1].accessDate);

		// High word of first cluster
		entryData->startCluster =
			((unsigned) dirEntry[count1].firstClusterHi << 16);

		// Last modified time
		newItem->modifiedTime = makeSystemTime(dirEntry[count1].modTime);

		// Last modified date
		newItem->modifiedDate = makeSystemDate(dirEntry[count1].modDate);

		// Low word of first cluster
		entryData->startCluster |= (unsigned) dirEntry[count1].firstClusterLo;
		...
		if (entryData->attributes & FAT_ATTRIB_SUBDIR)
			...
			newItem->size = dirEntry[count1].size;
...
static int readRootDir(fatInternalData *fatData, kernelDisk *theDisk)
{
	...
	// The root directory scheme is different depending on whether this is a
	// FAT32 volume or a FAT12/16 volume.  If it is not FAT32, then the root
	// directory is in a fixed location, with a fixed size (which we can
	// determine using values in the fatData structure).  If the volume is
	// FAT32, then we can treat it like a regular directory, with the starting
	// cluster number in the fatData->rootDirClusterF32 field.

	if ((fatData->fsType == fat12) || (fatData->fsType == fat16))
	{
		// Allocate a directory buffer based on the number of bytes per sector
		// and the number of FAT sectors on the volume.  Since we are working
		// with a FAT12 or FAT16 volume, the root directory size is fixed.
	...
	// Here again, we diverge depending on whether this is FAT12/16 or FAT32.
	// For FAT12/16, we will be reading consecutive sectors from the front
	// part of the disk.  For FAT32, we will be treating the root directory
	...
	else // if (fatData->fsType == fat32)
	{
		...
		// The only thing the read function needs in this data structure is
	...
	// We have to read the directory and fill out the chain of its files/
	// subdirectories in the lists
	...
	kernelFileEntry *directory)
{
	...
	// We have to count the files in the directory to figure out how many
	// items to fill in
...
{
	// This function takes a packed-BCD time value in system format and
	// returns the equivalent in packed-BCD DOS format
...
{
	// This function takes a packed-BCD date value in system format and
	// returns the equivalent in packed-BCD DOS format
	...
	// Clear the year and reset it.  Year should be 7 bits in places 9-15.
	returnedDate &= 0x000001FF;
	returnedDate |= (temp << 9);
...
static int fillDirectory(fatInternalData *fatData,
	kernelFileEntry *currentDir, void *dirBuffer)
{
	...
	while (listItemPointer)
	{
			...
			strcmp((char *) listItemPointer->name, ".."))
		{
			...
			longFilenameSlots = ((fileNameLength + 12) / 13);

			// We must do a loop backwards through the directory slots before
			// this one, writing the characters of this long filename into the
			// appropriate slots

			dirEntry += (longFilenameSlots - 1);
			subEntry = (char *) dirEntry;
			longFilenamePos = 0;
			...
			for (count = 0; count < longFilenameSlots; count++)
			{
				...
				// Put the "long filename entry" attribute byte into the
				// attribute slot
				subEntry[0x0B] = 0x0F;
				...
				// Determine whether this was the last long filename entry for
				// this file.  If not, move to the previous entry and loop.
				if (count == (longFilenameSlots - 1))
					break;
				else
					subEntry -= sizeof(fatDirEntry);
		...
		dirEntry->name[0] = NULL;
		strncpy(dirEntry->name, shortAlias, FAT_8_3_NAME_LEN);

		// attributes (byte value)
		dirEntry->attrib = (unsigned char) entryData->attributes;

		// reserved (byte value)
		dirEntry->res = (unsigned char) entryData->res;

		// timeTenth (byte value)
		dirEntry->createTimeTenth = (unsigned char) entryData->timeTenth;

		// Creation time (word value)
		dirEntry->createTime = makeDosTime(realEntry->creationTime);

		// Creation date (word value)
		dirEntry->createDate = makeDosDate(realEntry->creationDate);

		// Accessed date (word value)
		dirEntry->accessDate = makeDosDate(realEntry->accessedDate);

		// High word of first cluster (word value)
		dirEntry->firstClusterHi = (entryData->startCluster >> 16);

		// Last modified time (word value)
		dirEntry->modTime = makeDosTime(realEntry->modifiedTime);

		// Last modified date (word value)
		dirEntry->modDate = makeDosDate(realEntry->modifiedDate);

		// Low word of first cluster (word value)
		dirEntry->firstClusterLo = (entryData->startCluster & 0xFFFF);

		// Now we get the size.  If it's a directory we write zero for the
		// size (doubleword value).
		if (entryData->attributes & FAT_ATTRIB_SUBDIR)
			dirEntry->size = 0;
		else
			dirEntry->size = realEntry->size;
...
static int writeDir(kernelFileEntry *directory)
{
	...
	else
	{
		...
		dirBufferSize = (directoryEntries * sizeof(fatDirEntry));
		if (dirBufferSize % clusterSize)
			dirBufferSize += (clusterSize - (dirBufferSize % clusterSize));

		// Calculate the new number of blocks that will be occupied by this
		// directory
	...
	if (!dirBuffer)
	{
		kernelError(kernel_error, "Memory allocation error writing "
			"directory");
		return (status = ERR_MEMORY);
	}
...
static int checkFileChain(fatInternalData *fatData, kernelFileEntry *checkFile)
{
	...
	// Make sure there is supposed to be a starting cluster for this file.  If
	// not, make sure that the file's size is supposed to be zero.
	...
	int check, progress *prog)
{
	...
	for (count = 1; count < numClusters; count ++)
	{
		...
		if (nextClusterNumber > (clusterNumber + 1))
		{
			...
			{
				snprintf((char *) prog->statusMessage,
					PROGRESS_MAX_MESSAGELEN, _("Defragmenting %llu/%llu: %s"),
					(prog->numFinished + 1), prog->numTotal, entry->name);
				kernelLockRelease(&prog->lock);
			}
...
static unsigned getLastUsedCluster(fatInternalData *fatData)
{
	...
	// Move backwards through the free cluster bitmap.  When we find a non-
	// free cluster, return the cluster number.
...
static int checkFilename(volatile char *fileName)
{
	...
	// Loop through the entire filename, looking for any illegal characters
...
static int makeShortAlias(kernelFileEntry *theFile)
{
	...
	// Microsoft is innovating into the future of innovation.  (That's 7
	// points for me, plus I get an extra one for using four different forms
	// of the word).
	if (shortened)
	{
...
static int detect(kernelDisk *theDisk)
{
	...
	// The bytes-per-sector field may only contain one of the following
	// values: 512, 1024, 2048 or 4096.  Anything else is illegal according to
	// MS.  512 is almost always the value found here.
	...
	// should be able to find the substring "FAT" in the first 3 bytes of the
	// fileSysType field.
	...
	{
		// We will say this is not a FAT filesystem.  We might be wrong, but
		// we can't be sure otherwise.
		return (status = 0);
	}

	// NTFS $Boot files look a lot like FAT, and we in fact can reach this
	// point without failing.  Check for NTFS signature.
	...
	int longFormat, progress *prog)
{
	...
	{
		fatData.rootDirSects = (((sizeof(fatDirEntry) *
			fatData.bpb.rootEntCount) + (fatData.bpb.bytesPerSect - 1)) /
			fatData.bpb.bytesPerSect);
	...
	if ((fatData.fsType == fat12) || (fatData.fsType == fat16))
	{
		...
		strncpy((char *) fatData.bpb.fat.volumeLabel, label,
			FAT_8_3_NAME_LEN);
	...
	if (fatData.fsType == fat32)
	{
		// These fields are specific to the FAT32 filesystem type, and are not
		// applicable to FAT12 or FAT16
	...
	kernelLog("Format: Type: %s  Total Sectors: %u  Bytes Per Sector: %u  "
		"Sectors Per Cluster: %u  Root Directory Sectors: %u  Fat Sectors: "
		"%u  Data Clusters: %u", theDisk->fsType, fatData.totalSects,
		fatData.bpb.bytesPerSect, fatData.bpb.sectsPerClust,
		fatData.rootDirSects, fatData.fatSects, fatData.dataClusters);
	...
	uquad_t *maxBlocks, progress *prog)
{
	...
	*minBlocks = (minSysSects + minDataSects);
	...
	// What is the maximum number of data sectors, based on the maximum
	// cluster number for the FAT size?
	maxDataSects = ((fatData->terminalClust - 2) *
...
static int resize(kernelDisk *theDisk, uquad_t blocks, progress *prog)
{
	...
	if (diffFatSects)
	{
		...
		status = moveData(fatData, oldStartSector, newStartSector,
			moveSectors, prog);
		if (status < 0)
		{
		...
		// If we are expanding the filesystem, we need to clear the new FAT
		// sectors
		if (diffFatSects > 0)
		{
			if (prog && (kernelLockGet(&prog->lock) >= 0))
...
static int unmount(kernelDisk *theDisk)
{
	...
	// Everything should be cozily tucked away now.  We can safely discard the
	// information we have cached about this filesystem.
	freeFatData(theDisk);
...
static int newEntry(kernelFileEntry *entry)
{
	...
	// attach FAT-specific data to the file entry
...
static int inactiveEntry(kernelFileEntry *entry)
{
	...
	// deallocate our FAT-specific data from the file entry.
...
{
	// This function is called by the filesystem manager whenever a file has
	// been moved from one place to another.  This allows us the chance to do
	// any FAT-specific things to the file that are necessary.  In our case,
	// we need to re-create the file's short alias, since this is directory-
	// dependent.
...
static int timestamp(kernelFileEntry *theFile)
{
	...
	// The only FAT-specific thing we're doing here is setting the 'archive'
	// bit
