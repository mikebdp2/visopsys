...
static unsigned gptHeaderChecksum(gptHeader *header)
{
	...
	oldChecksum = header->headerCRC32;
	header->headerCRC32 = 0;

	// Get the checksum
	checksum = kernelCrc32(header, header->headerBytes, NULL);

	header->headerCRC32 = oldChecksum;
	...
	kernelDisk *newLogicalDisks[], int *newLogicalDiskCounter)
{
	...
	header = kernelMalloc(physicalDisk->sectorSize);
	if (!header)
		return (status = ERR_MEMORY);

	// Read the header.  The guard MS-DOS table is in the first sector.  Read
	// the second sector.
	status = kernelDiskReadSectors((char *) physicalDisk->name, 1, 1, header);
	if (status < 0)
	{
		kernelFree(header);
		return (status);
	...
	entrySectors = ((entryBytes + (physicalDisk->sectorSize - 1)) /
			physicalDisk->sectorSize);

	// Allocate a buffer for reading the entries
	entries = kernelMalloc(entrySectors * physicalDisk->sectorSize);
	if (!entries)
		return (status = ERR_MEMORY);
	...
	status = kernelDiskReadSectors((char *) physicalDisk->name,
		(unsigned) entriesLogical, entrySectors, entries);
	if (status < 0)
	{
		kernelFree(entries);
		return (status);
		...
		(physicalDisk->numLogical < DISK_MAX_PARTITIONS)); count ++)
	{
		...
		typeGuid = (guid *) &entries[count].typeGuid;
		...
		if (kernelDiskGetGptPartType(typeGuid, &gptType) >= 0)
		{
		...
		logicalDisk->startSector = (unsigned) entries[count].startingLBA;
		logicalDisk->numSectors = ((unsigned) entries[count].endingLBA -
	...
	kernelDisk *newLogicalDisks[], int *newLogicalDiskCounter)
{
	...
	int status = 0;
	msdosMbr *mbr = NULL;
	msdosEntry *entries = NULL;
	msdosEntry *extendedRecord = NULL;
	kernelDisk *logicalDisk = NULL;
	...
	mbr = kernelMalloc(physicalDisk->sectorSize);
	if (!mbr)
		return (status = ERR_MEMORY);

	// Read the first sector of the disk
	status = kernelDiskReadSectors((char *) physicalDisk->name, 0, 1, mbr);
	if (status < 0)
	{
		kernelFree(mbr);
		return (status);
	...
	while (physicalDisk->numLogical < DISK_MAX_PARTITIONS)
	{
		...
		// Set this pointer to the first partition entry in the master boot
		// record
		entries = mbr->partTable.entries;
		...
		for (count = 0; count < MSDOS_TABLE_ENTRIES; count ++)
		{
			...
			if (MSDOSTAG_IS_EXTD(entries[count].tag))
			{
				extendedRecord = &entries[count];
				continue;
			}
			...
			// Now try to figure out the real one
			kernelDiskGetMsdosPartType(entries[count].tag, &msdosType);
			...
			logicalDisk->startSector = (startSector +
				entries[count].startLogical);
			logicalDisk->numSectors = entries[count].sizeLogical;
			...
			*newLogicalDiskCounter += 1;
			...
			// partitions
			if ((entries[count].endHead >= physicalDisk->heads) ||
				((entries[count].endCylSect & 0x3F) >
				physicalDisk->sectorsPerCylinder))
			{
				physicalDisk->heads = (entries[count].endHead + 1);
				physicalDisk->sectorsPerCylinder =
					(entries[count].endCylSect & 0x3F);
		...
		// It can happen.
		if (extendedStartSector && ((extendedRecord->startLogical +
		...
		// well.
		startSector = extendedRecord->startLogical;
...
static int identifyBootCd(void)
{
	...
	isoBootRecordDescriptor *bootRecDesc = NULL;
	isoBootCatInitEntry *bootCatEntry = NULL;
	fatBootSect *fatBoot = NULL;
	...
	for (count = 0; count < physicalDiskCounter; count ++)
	{
		...
		bootRecDesc = (isoBootRecordDescriptor *) buffer;
		status = readWrite(physicalDisk, bootRecDesc->bootCatSector, 1,
			buffer, IOMODE_READ);
		if (status < 0)
		{
			kernelLockRelease(&physicalDisk->lock);
		...
		// Check whether the disk is bootable
		...
		// This should be a Visopsys FAT boot sector
		fatBoot = (fatBootSect *) buffer;

		// Check the standard FAT boot sector signature
		if (fatBoot->bootSig != MSDOS_BOOT_SIGNATURE)
		{
		...
		// Check the Visopsys boot sector signature.  This is particular to
		// Visopsys FAT boot sectors.  Should be the same signature as the one
		// passed by the OS loader.
		if (fatBoot->bootSectorSig == kernelOsLoaderInfo->bootSectorSig)
		{
		...
		else
		{
			kernelDebug(debug_io, "Disk %s boot sector signature (0x%08x) "
				"doesn't match", physicalDisk->name, fatBoot->bootSectorSig);
			kernelFree(buffer);
		}
	}
...
static int identifyBootDisk(void)
{
	...
	for (count = 0; count < logicalDiskCounter; count ++)
	{
		...
		status = readWrite(physicalDisk, logicalDisk->startSector, 1, mbr,
			IOMODE_READ);
		...
		if (mbr->diskSig == kernelOsLoaderInfo->bootSectorSig)
		{
			// We found the logical disk we booted from
			kernelDebug(debug_io, "Disk %s boot sector signature matches",
				logicalDisk->name);
		...
		{
			kernelDebug(debug_io, "Disk %s boot sector signature (0x%08x) "
				"doesn't match", logicalDisk->name, mbr->diskSig);
			kernelFree(mbr);
		}
	}
