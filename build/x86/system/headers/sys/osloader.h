...
// This header file contains information about the structures passed to the
// kernel by the OS loader at startup, and anything else specific to the
// OS loader and Visopsys boot sectors
...
// The data structure created by the loader to describe the particulars about
// the current graphics environment to the kernel
typedef struct {
...
// The data structure created by the loader to describe the particulars about
// a floppy disk drive to the kernel
typedef struct {
	int type;
	int heads;
	int tracks;
...
// The Visopsys-specific FAT boot sector
typedef struct {
	unsigned char bootcode[498];
	unsigned bootSectorSig;
	unsigned loaderStartCluster;
	unsigned loaderNumSectors;
